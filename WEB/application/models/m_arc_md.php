...
class  M_arc_md extends CI_Model {
	...
	if (!empty($empId)){
	    $res = $this->jservice->get(array('Action' => 'GetRowData','FieldList'=>'B2G(arc_mdid) as arc_mdid, B2G(arc_mdid) as id,B2G(instanceid) as instanceid, arc_md_BRIEF_F(arc_mdid , NULL) as  brief,B2G(mymodule) mymodule, armd_info_BRIEF_F(mymodule, NULL) as mymodule_grid,name,deivetype,theaddress,master_fio', 'PartName' => 'arc_md', 'ID' =>  $empId 	));
	    if (!empty($res)) {
	        $result = $res[0];
	    }
	}
    ...
    function getRows($sort=array())
		{
	    $res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(arc_mdid) as arc_mdid, B2G(arc_mdid) as id,B2G(instanceid) as instanceid, arc_md_BRIEF_F(arc_mdid , NULL) as  brief,B2G(mymodule) mymodule, armd_info_BRIEF_F(mymodule, NULL) as mymodule_grid,name,deivetype,theaddress,master_fio', 'ViewName' => 'arc_md'));
	    if (count($res)) {
	        return $res;
	    } else {
	        return null;
    ...
    function getRowsByInstance($id,$sort=array())
		{
	$res = $this->jservice->get(array('Action' => 'GetViewData','Sort'=>$sort,'FieldList'=>'B2G(arc_mdid) as arc_mdid, B2G(arc_mdid) as id,B2G(instanceid) as instanceid, arc_md_BRIEF_F(arc_mdid , NULL) as  brief,B2G(mymodule) mymodule, armd_info_BRIEF_F(mymodule, NULL) as mymodule_grid,name,deivetype,theaddress,master_fio', 'ViewName' => 'arc_md', 'WhereClause' => 'instanceid=G2B(\''. $id . '\')'));
	if (count($res) == 0) {
	    return null;
	} else {
	    return $res;
